...
{
	if ((point.Z < Data.RD_W - Data.MOVING_AREA_Z) || (point.Z >= Data.HomePosition.Z))
		return false;

	if (sqrt(point.X * point.X + point.Y * point.Y) > Data.MOVING_AREA_LARGEST_DIAMETER)
	{
		return false;
	}
...
{
	if ((angle.Theta1 < (THETA1_HOME_POSITION)) || (angle.Theta1 > (THETA1_MAX_POSITION)))
		return false;
	if ((angle.Theta2 < (THETA2_HOME_POSITION)) || (angle.Theta2 > (THETA1_MAX_POSITION)))
		return false;
	if ((angle.Theta3 < (THETA3_HOME_POSITION)) || (angle.Theta3 > (THETA1_MAX_POSITION)))
		return false;
	return true;
}
...
bool ToolClass::Wait()
{
		if (HAL_GetTick() - LastTime < TimeDelay)
		{
			return false;
		}
		else
